...
#author          JB, AW
#date            2019 04 and 2021 05
#version         V8
#usage           ./v8.cron-job-black-list-manager.sh
...
# fixes for the SWX lists
#
######################################################
sync_file () {
publish_dir="/var/www/html/e-edl"
# pause, add cp
...
# 2012 big updates due to SWX formating new cleaners add
...
# 99 perfcent of the time it needs cleaning ...
# cleaning is compund, one then enxt, not should only
# hit one,  if have them as atomic inspection
# and save back to url_item not a temp, save on ram.
# use cuts and subistutes to update temp, item recives from temp
# file based for now, some day may need to make
# in to hash for speed

#######################################
# swx sources
patt=" #"
[[ ${url_item} =~ ${patt} ]] && {
url_temp=$(echo ${url_item%%#*})
url_item=${url_temp}
...
url_temp="${url_item:8}"
url_item=${url_temp}
#echo "IPL $url_temp" >> $TEMP
#
}

#######################################
# if no cleaner hit then should be ok, so cleaners are above.
echo "${url_item}" >> $TEMP
...
# final list is blockurl.txt ,collect the bl souces lists, uniq each, then uniq the one, reduction was about 10 to 30 perfect, fyi
##
